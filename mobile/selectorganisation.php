...
$camp = db_value('SELECT c.id AS camp_id
	...
	WHERE q.code = :barcode AND q.id = s.qr_id AND l.id = s.location_id AND c.id = l.camp_id', array('barcode' => $_GET['barcode']));
if ($camp) redirect('?camp=' . $camp . '&barcode=' . $_GET['barcode']);
