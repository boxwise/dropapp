<?php
	...
	$data['products'] = db_array('SELECT p.id AS value, CONCAT(p.name, " " ,IFNULL(g.label,"")) AS label FROM products AS p LEFT OUTER JOIN genders AS g ON p.gender_id = g.id WHERE NOT p.deleted AND camp_id = :camp_id ORDER BY name',array('camp_id'=>$_SESSION['camp']['id']));
