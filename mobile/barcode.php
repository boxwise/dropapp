<?php
	...
	if($_GET['barcode'] && !db_value('SELECT id FROM qr WHERE code = :code',array('code'=>$_GET['barcode']))) {
		...
		if($_GET['boxid']) {
			$box = db_row('SELECT s.*, c.id AS camp_id, c.name AS campname, CONCAT(p.name," ",g.label," ",IFNULL(s2.label, "")," (",p.comments,")") AS product,s.comments as comment, l.label AS location FROM stock AS s
