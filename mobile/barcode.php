<?php
    ...
    if ($_GET['barcode'] && !db_value('SELECT id FROM qr WHERE code = :code', ['code' => $_GET['barcode']])) {
        $data['warning'] = true;
		...
		WHERE (NOT s.deleted OR s.deleted IS NULL) AND s.id = :id', ['id' => $_GET['boxid']]);
        } else {
            $box = db_row('SELECT s.*, c.id AS camp_id, c.name AS campname, CONCAT(p.name," ",g.label," ",IFNULL(s2.label, "")) AS product, p.name AS product2, g.label AS gender, IFNULL(s2.label, "") AS size, l.label AS location FROM stock AS s
		LEFT OUTER JOIN products AS p ON p.id = s.product_id
		LEFT OUTER JOIN genders AS g ON g.id = p.gender_id
		...
		LEFT OUTER JOIN qr AS q ON q.id = s.qr_id
		LEFT OUTER JOIN camps AS c ON c.id = l.camp_id
		WHERE q.code = :barcode', ['barcode' => $data['barcode']]);
        }
        if ('0000-00-00 00:00:00' != $box['deleted'] && !is_null($box['deleted'])) {
            ...
            if ($box['id']) {
                ...
                $locations = db_array('SELECT id AS value, label, IF(id = :location, true, false) AS selected FROM locations WHERE camp_id = :camp_id ORDER BY seq', ['camp_id' => $_SESSION['camp']['id'], 'location' => $box['location_id']]);
