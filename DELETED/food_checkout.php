<?php
	...
	if(!$ajax) {
		...
		$query='SELECT fd.id as id, fd.label as label, fd.created AS start_date,
			(SELECT COUNT(pc.id) 
				FROM people AS pc
				WHERE pc.camp_id = '.$_SESSION['camp']['id'].' AND COALESCE(pc.created,0) < fd.created AND ((NOT pc.deleted AND pc.visible) OR fd.created < pc.deleted) AND (YEAR(NOW()) - YEAR(pc.date_of_birth) - (DAYOFYEAR(NOW()) > DAYOFYEAR(pc.date_of_birth))) < '.$_SESSION['camp']['adult_age'].') AS children,
			(SELECT COUNT(pa.id) 
				FROM people AS pa
				WHERE pa.camp_id = '.$_SESSION['camp']['id'].' AND pa.created < fd.created AND ((NOT pa.deleted AND pa.visible) OR fd.created < pa.deleted) AND (YEAR(NOW()) - YEAR(pa.date_of_birth) - (DAYOFYEAR(NOW()) > DAYOFYEAR(pa.date_of_birth))) > '.$_SESSION['camp']['adult_age'].') AS adults';
		for ($i=1; $i<6; $i++){
