...
{/if}
	...
	<span class="men">{($data['totalmen'])}</span> of our community are male (<span class="men">{$data['menperc']|round}%</span>) and <span class="women">{$data['totalwomen']}</span> are female (<span class="women">{$data['womenperc']|round}</span>%).<br />Of these people, <span class="number">{$data['children']}</span> are {$_SESSION['camp']['adult-age']-1} or younger (<span class="number">{($data['children']/$data['residents']*100)|round}%</span>).<br /><span class="number">{$data['under18']}</span> are under 18 (<span class="number">{($data['under18']/$data['residents']*100)|round}%</span>).</h1>
<hr />
...
<h1 class="light">The current laundry cycle (of two weeks) started <span class="number">{$settings['laundry_cyclestart']|date_format:"%d-%m-%Y"}</span>. Until now <span class="number">{$data['laundry_appointments']}</span> appointments have been made, using <span class="number">{($data['laundry_appointments']/$data['laundry_slots']*100)|number_format:0:",":"."}%</span> of the capacity. We have <span class="number">{$data['laundry_noshow']}</span> no-shows (<span class="number">{($data['laundry_noshow']/$data['laundry_appointments']*100)|number_format:0:",":"."}%</span>). We have served <span class="number">{$data['laundry_beneficiaries']|intval}</span> beneficiaries.</h1>
