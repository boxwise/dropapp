<div class="content-form">
	<div class="row row-title">
		...
		<h3 class="col-md-6 col-sm-12">Make sure that the user brings back the bicycle in good condition, that they locked it correctly and that you get key, front light, reflective vest and helmet back. After checking all this, give the user their Bike Certificate card back.</h3>
	</div>
				...
				<table class="table" {if $element['sortlist']}data-sortlist="{$element['sortlist']}"{/if}>
					...
					<tbody>
				      {foreach $element['data'] as $row}
						...
						{foreach $element['columns'] as $key=>$column name="rows"}
									...
									<div class="td-content">
										...
										<label class="item-select-label"><input class="item-select" type="checkbox"></label>
											{if $element['allowedit'] or !isset($element['allowedit'])}
												<a class="td-content-field data-field-{$key}" href="?action={$element['action']}_edit{if $element['modal']}&modal=1{/if}&id={$row['id']}">
											{else}
												<p class="td-content-field data-field-{$key}">
											{/if}
											{$row[$key]|strip_tags:false|truncate}
											{if $element['allowedit'] or !isset($element['allowedit'])}
												</a>
											{else}
												</p>
...
<div class="fc"></div>
<a href="?action=bicycles_edit&return={$data['id']}&user={$data['people_id']}" class="btn btn-success">Return the bike</a>
</div>
