	<div class="form-group{if $element['hidden']} hidden{/if}" id="div_{$element['field']}">
		...
		<div class="col-sm-{if $element['width']>0 and $element['width']<11}{$element['width']}{else}10{/if} input-element {if $element['tooltip']}has-tooltip{/if}">
			<textarea name="{$element['field']}" {if $element['maxlength']}data-max-count="{$element['maxlength']}"{/if} rows="{if $element['rows']}{$element['rows']}{else}3{/if}" id="field_{$element['field']}" class="form-control" {if $element['readonly']}readonly{/if} {if $element['required']}required{/if} {if $element['onblur']}onblur="{$element['onblur']};"{/if} {if $element['onchange']}onchange="{$element['onchange']};"{/if} {if $element['onkeyup']}onkeyup="{$element['onkeyup']};"{/if} {if isset($element['testid'])}data-testid="{$element['testid']}"{/if}>{$data[$element['field']]}</textarea>
