	<div class="form-group{if $element['hidden']} hidden{/if}" id="div_{$element['field']}">
		...
		<div class="col-sm-{if $element['width']>0 and $element['width']<11}{$element['width']}{else}6{/if} {if $element['tooltip']}has-tooltip{/if}">
			<input type="password" id="field_{$element['field']}" name="{$element['field']}" {if $element['minlength']}minlength="{$element['minlength']}"{/if} {if $element['pwcheck']}pwcheck="true"{/if} class="form-control" {if $element['onblur']}onblur="{$element['onblur']};"{/if} {if $element['onchange']}onchange="{$element['onchange']};"{/if} {if $element['onkeyup']}onkeyup="{$element['onkeyup']};"{/if} {if $element['disableautocomplete']}autocomplete="new-password"{/if} {if $element['readonly'] || $element['locked']}readonly{/if} {if $element['required']}required{/if} {if isset($element['testid'])}data-testid="{$element['testid']}"{/if}>
