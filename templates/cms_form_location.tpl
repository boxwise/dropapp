	<div class="form-group{if $element['hidden']} hidden{/if}" id="div_{$element['field']}">
		...
		<div class="col-sm-{if $element['width']>0 and $element['width']<11}{$element['width']}{else}6{/if}">
			{if $element['locked']}<div class="input-group locked">{/if}
	 		<input type="text" id="field_{$element['field']}" name="{$element['field']}" {if $element['maxlength']}data-max-count="{$element['maxlength']}"{/if} class="form-control" value="{$data[$element['field']]}" {if isset($element['onchange']) or $element['format']}onchange="{if $element['format']}cms_form_{$element['format']}('{$element['field']}');{/if}{$element['onchange']};"{/if} onkeyup="setExternalText(this, '#test');{$element['onkeyup']};" onblur="setExternalInput(this, '#test');{$element['onblur']};" {if $element['minlength']}minlength="{$element['minlength']}"{/if} {if $element['readonly'] || $element['locked']}readonly{/if} {if $element['required']}required{/if} {if isset($element['testid'])}data-testid="{$element['testid']}"{/if}>
