<div class="info-aside" id="people_id_selected">
	...
	{if $smarty.session.camp['food']}<p class="familycredit{if $data['fooddrops']<=0} warningbox{/if}">Max food: <i class="fa fa-tint"></i> <span id="foodcredit"  data-food-credit="{$data['fooddrops']}">{$data['fooddrops']}</span> drops <span class="people-comment">This is the amount of people's drops that they can spend on food items.</span></p>
	{/if}
