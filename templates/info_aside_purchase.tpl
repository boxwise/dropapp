<div class="info-aside" id="people_id_selected">{$data['test']}
	...
	{if ((isset($data['approvalsigned']) || {$smarty.get.origin} == "start") && {$data['approvalsigned']} == FALSE)}<a class="btn btn-danger" type="button" onclick="location.href='?action=people_edit&amp;id={$person['id']}'" title="Please have the familyhead/beneficiary read and sign the approval form for storing and processing their data."><span class="fa fa-edit"> </span> Privacy declaration</a>{/if}
