<?php
	...
	$result = db_query('SELECT FLOOR((COUNT(id) + SUM(extraportion))/3) AS green, (COUNT(id) + SUM(extraportion))%3 AS red FROM people WHERE visible AND camp_id = '.$_SESSION['camp']['id'].' AND NOT deleted GROUP BY container');
	...
	$data[0]['adults'] = db_value('SELECT COUNT(id) FROM people WHERE visible AND camp_id = '.$_SESSION['camp']['id'].' AND NOT deleted  AND DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), date_of_birth)), "%Y")+0 >= '.$_SESSION['camp']['adult_age']);
	$data[0]['children'] = db_value('SELECT COUNT(id) FROM people WHERE visible AND camp_id = '.$_SESSION['camp']['id'].' AND NOT deleted AND DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), date_of_birth)), "%Y")+0 < '.$_SESSION['camp']['adult_age']);
