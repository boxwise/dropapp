...
if (isset($_SESSION['camp']['id'])) {
    $data['items'] = intval(db_value('SELECT SUM(s.items) FROM (stock AS s, products AS p) WHERE s.product_id = p.id AND (NOT p.deleted OR p.deleted IS NULL) AND (NOT s.deleted OR s.deleted IS NULL) AND s.box_state_id = 1 AND p.camp_id = :camp_id', ['camp_id' => $_SESSION['camp']['id']]));
    $data['boxes'] = db_value('SELECT COUNT(s.id) FROM (stock AS s, products AS p) WHERE s.product_id = p.id AND (NOT p.deleted OR p.deleted IS NULL) AND (NOT s.deleted OR s.deleted IS NULL) AND s.box_state_id = 1 AND p.camp_id = :camp_id ', ['camp_id' => $_SESSION['camp']['id']]);
    ...
    while (strtotime($date) <= strtotime($end)) {
        ...
        if ($sales) {
            ...
            $data['sales'][$dateFormatted] = db_value('SELECT SUM(t.count) AS aantal FROM (transactions AS t, people AS pp)
        ...
        while ($borrow = db_fetch($result)) {
            ...
            $data['borrow'][(new DateTime($date))->format('D j M')][$borrow['label']] = $borrow;
        }
        $date = date('Y-m-d', strtotime('+1 day', strtotime($date)));
    }

    $data['weeklabel'] = ((new DateTime())->format('w') ? 'Last' : 'This');
    ...
    $previous = (new DateTime($_SESSION['camp']['laundry_cyclestart']))->sub(new DateInterval('P14D'))->format('Y-m-d');
