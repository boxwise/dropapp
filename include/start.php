<?php
	...
	if($_SESSION['camp']['market']) {
		...
		$data['containers'] = db_value('SELECT COUNT(DISTINCT(container)) FROM people WHERE visible AND NOT deleted AND camp_id = :camp_id',array('camp_id'=>$_SESSION['camp']['id']));
		$data['containerscamp'] = db_value('SELECT COUNT(DISTINCT(container)) FROM people WHERE NOT deleted AND visible AND camp_id = :camp_id AND LEFT(container,2) != "PK"',array('camp_id'=>$_SESSION['camp']['id']));
		$data['containersoutside'] = db_value('SELECT COUNT(DISTINCT(container)) FROM people WHERE NOT deleted AND visible AND camp_id = :camp_id AND LEFT(container,2) = "PK"',array('camp_id'=>$_SESSION['camp']['id']));
	
		$data['adults'] = db_value('SELECT COUNT(id) FROM people WHERE visible AND camp_id = :camp_id AND NOT deleted AND DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), date_of_birth)), "%Y")+0 >= '.$_SESSION['camp']['adult-age'],array('camp_id'=>$_SESSION['camp']['id']));
		$data['children'] = db_value('SELECT COUNT(id) FROM people WHERE visible AND camp_id = :camp_id AND NOT deleted AND DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), date_of_birth)), "%Y")+0 < '.$_SESSION['camp']['adult-age'],array('camp_id'=>$_SESSION['camp']['id']));
...
FROM borrow_transactions AS b1 LEFT OUTER JOIN borrow_items AS i ON i.id = b1.bicycle_id LEFT OUTER JOIN borrow_categories AS c ON c.id = i.category_id LEFT OUTER JOIN people AS p ON p.id = b1.people_id WHERE b1.status = "out" AND DATE_FORMAT(b1.transaction_date,"%Y-%m-%d") = :date GROUP BY p.gender, c.id', array('date'=>$date));
		...
		$data['laundry_appointments'] = db_value('SELECT COUNT(id) FROM laundry_appointments WHERE people_id > 0 AND cyclestart = :cyclestart',array('cyclestart'=>$_SESSION['camp']['laundry_cyclestart']));
		$data['laundry_slots'] = db_value('SELECT COUNT(id) FROM laundry_slots');
		$data['laundry_noshow'] = db_value('SELECT COUNT(id) FROM laundry_appointments WHERE cyclestart = :cyclestart AND noshow',array('cyclestart'=>$_SESSION['camp']['laundry_cyclestart']));
		$result = db_query('SELECT (SELECT COUNT(id) FROM people WHERE id = people_id OR parent_id = people_id) AS c FROM laundry_appointments WHERE people_id > 0 AND cyclestart = :cyclestart GROUP BY people_id', array('cyclestart'=>$_SESSION['camp']['laundry_cyclestart']));
		...
		$result = db_query('SELECT (SELECT COUNT(id) FROM people WHERE id = people_id OR parent_id = people_id) AS c FROM laundry_appointments WHERE people_id > 0 AND cyclestart = :cyclestart GROUP BY people_id', array('cyclestart'=>$previous));
