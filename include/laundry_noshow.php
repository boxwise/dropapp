...
$cyclestart = date('Y-m-d', strtotime('+'.$offset.' days', strtotime((string) $_SESSION['camp']['laundry_cyclestart'])));
...
switch ($field) {
    ...
    case 'dropoff':
    case 'collected':
        db_query('UPDATE laundry_appointments SET '.$field.' = NOT '.$field.' WHERE id = :id', ['id' => $data['id']]);
}
