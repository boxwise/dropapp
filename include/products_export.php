<?
	...
	$result = db_query('SELECT products.*, sg.label AS sizegroup, g.label AS gender, CONCAT(products.value," '.$translate['market_coins'].'") AS drops, COALESCE(SUM(s.items),0) AS items, IF(SUM(s.items),1,0) AS preventdelete, pc.label AS category FROM products
	...
	echo '"Product";"Gender";"Size group";"Items";"'.ucfirst($translate['market_coins']).'"'."\n";
