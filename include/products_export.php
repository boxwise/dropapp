...
$result = db_query('SELECT p.*, sg.label AS sizegroup, g.label AS gender, p.value AS drops, COALESCE(SUM(s.items),0) AS items, pc.label AS category 
		FROM products AS p
	...
	');
$keys = array("name"=>"Product", "category"=>"Category", "gender"=>"Target Group", "sizegroup"=>"Sizegroup", "items" => "Items", "drops"=>$_SESSION['camp']['currencyname']);
