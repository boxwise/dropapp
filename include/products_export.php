...
$result = db_query(
	...
	WHERE 
		...
		camp_id = ? '.($_SESSION['export_ids_products'] ? 'AND p.id IN ('.$id_pars.')' : 'AND FALSE').'
		GROUP BY p.id ORDER BY category_id, g.seq, name',
    $export_ids_array
);
unset($_SESSION['export_ids_products']);
$keys = ['name' => 'Product', 'category' => 'Category', 'gender' => 'Gender', 'sizegroup' => 'Sizegroup', 'items' => 'Items', 'drops' => $_SESSION['camp']['currencyname']];
