...
$camps = db_simplearray('SELECT c.id, c.name FROM camps AS c
		...
		GROUP BY c.id', ['camp_id' => $_SESSION['camp']['id'], 'organisation' => $_SESSION['organisation']['id']]);
$market = db_value('SELECT market FROM camps WHERE id = :camp', ['camp' => $_SESSION['camp']['id']]);
...
if (!$ajax) {
    ...
    listsetting('search', ['pro.name', 'g.label', 's.label']);
