<?php
    ...
    if (!$ajax) {
        ...
        // Shopping cart
        addfield('shopping_cart', '', '', ['width' => 15, 'columns' => ['product' => 'Product', 'count' => 'Amount', 'drops2' => 'Price', 'drops3' => 'Total Price', 'delete' => '']]);

        $table = 'transactions';
        addfield('title', 'Last Purchases');
        addfield('list', '', 'purch', ['width' => 10, 'query' => 'SELECT t.*, u.naam AS user, CONCAT(IF(drops>0,"+",""),drops) AS drops2, count /*AS countupdown*/, DATE_FORMAT(transaction_date,"%d-%m-%Y %H:%i") AS tdate, CONCAT(p.name, " " ,IFNULL(g.label,"")) AS product FROM transactions AS t LEFT OUTER JOIN cms_users AS u ON u.id = t.user_id LEFT OUTER JOIN products AS p ON p.id = t.product_id LEFT OUTER JOIN genders AS g ON p.gender_id = g.id 
