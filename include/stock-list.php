<?php
	...
	$query = 'SELECT CONCAT(p.id,"-",g.id,"-",IFNULL(s.id,"")) AS id,
		...
		g.label AS gender,
		s.label AS size,
		(SELECT SUM(st.items) FROM stock AS st, products AS p2, locations AS l WHERE st.product_id = p2.id AND p2.gender_id = g.id AND st.size_id = s.id AND st.location_id = l.id AND (NOT st.deleted OR st.deleted IS NULL) AND l.visible AND p.name = p2.name AND l.camp_id = '.$_SESSION['camp']['id'].') AS stock,
		...
		(IF(g.adult,DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), p2.date_of_birth)), "%Y")+0>='.$_SESSION['camp']['adult-age'].',0) OR IF(g.baby,DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), p2.date_of_birth)), "%Y")+0<2,0) OR IF(g.child,DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), p2.date_of_birth)), "%Y")+0 BETWEEN 2 AND '.$_SESSION['camp']['adult-age'].',0)))*IFNULL(s.portion,100)/100) AS target,
		p.amountneeded ';
