...
SELECT 
	...
	foreach($camps as $key=>$camp) {
		$insert[] = '
	(SELECT SUM(items) FROM stock AS st, locations AS l, products AS p2 WHERE st.product_id = p2.id AND st.location_id = l.id AND l.visible AND NOT st.deleted AND p.name = p2.name AND p2.gender_id = g.id AND st.size_id = s.id AND l.camp_id = '.$key.') AS "camp'.$key.'"';
	} 
	$query .= join(',',$insert);
