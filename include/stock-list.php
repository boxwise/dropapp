<?php
    ...
    $query = '
        SELECT CONCAT(p.id,"-",g.id,"-",IFNULL(s.id,"")) AS id,
            ...
            s.label AS size,
            (SELECT SUM(st.items) 
                FROM stock AS st, products AS p2, locations AS l 
                WHERE st.product_id = p2.id AND p2.gender_id = g.id AND st.size_id = s.id AND st.location_id = l.id AND (NOT st.deleted OR st.deleted IS NULL) AND l.visible AND l.deleted IS NULL AND p.name = p2.name AND l.camp_id = '.$_SESSION['camp']['id'].') AS stock,
            COALESCE( NULLIF( (SELECT COUNT(st.id) FROM stock AS st, products AS p2, locations AS l WHERE st.product_id = p2.id AND p2.gender_id = g.id AND st.size_id = s.id AND st.location_id = l.id AND (NOT st.deleted OR st.deleted IS NULL) AND l.visible AND l.deleted IS NULL AND p.name = p2.name AND l.camp_id = '.$_SESSION['camp']['id'].'),0),"") AS boxes,
    ...
    foreach ($camps as $key => $camp) {
        $insert[] = ', (SELECT SUM(items) FROM stock AS st, locations AS l, products AS p2 WHERE st.product_id = p2.id AND st.location_id = l.id AND l.visible AND l.deleted IS NULL AND (NOT st.deleted OR st.deleted IS NULL) AND p.name = p2.name AND p2.gender_id = g.id AND st.size_id = s.id AND l.camp_id = '.$key.') AS "camp'.$key.'"';
    }
    $query .= join(',', $insert);
