<?
	...
	$result = db_query('SELECT ls.id AS timeslot, ls.day, ls.time, ls.machine, lm.label AS machinename, la.noshow, p.* FROM laundry_slots AS ls LEFT OUTER JOIN laundry_appointments AS la ON la.timeslot = ls.id AND cyclestart = :cyclestart LEFT OUTER JOIN people AS p ON p.id = la.people_id LEFT OUTER JOIN laundry_machines AS lm ON lm.id = ls.machine ORDER BY ls.id ',array('cyclestart'=>$settings['laundry_cyclestart']));
