<?php
	...
	if($_POST) {
		...
		if($type=='graph') {
			...
			while (strtotime($date) <= strtotime($end)) {
				...
				if($sales) {
					$test = db_simplearray('SELECT p.groupname AS gender, SUM(t.count) AS aantal FROM (transactions AS t, people AS pp)
	...
	GROUP BY p.groupname ORDER BY SUM(t.count)',array('camp_id'=>$_SESSION['camp']['id']));
			...
			$totalsales = db_value('
	SELECT SUM(t.count) AS aantal FROM transactions AS t, people AS p
	WHERE t.people_id = p.id AND p.camp_id = :camp_id AND t.product_id > 0 AND t.transaction_date >= "'.$start.' 00:00" AND t.transaction_date <= "'.$end.' 23:59"',array('camp_id'=>$_SESSION['camp']['id']));
			$totaldrops = -1*db_value('
	SELECT SUM(t.drops) AS aantal FROM transactions AS t, people AS p
	WHERE t.people_id = p.id AND p.camp_id = :camp_id AND t.product_id > 0 AND t.transaction_date >= "'.$start.' 00:00" AND t.transaction_date <= "'.$end.' 23:59"',array('camp_id'=>$_SESSION['camp']['id']));

			if($type=='gender') {
				$data = getlistdata('
		SELECT g.label AS gender, SUM(t.count) AS aantal FROM (transactions AS t, people AS pp)
		LEFT OUTER JOIN products AS p ON t.product_id = p.id
		...
		GROUP BY p.gender_id');
				...
				$data = getlistdata('
	SELECT p.name, g.label AS gender, SUM(t.count) AS aantal FROM (transactions AS t, people AS pp)
	LEFT OUTER JOIN products AS p ON t.product_id = p.id
	LEFT OUTER JOIN genders AS g ON p.gender_id = g.id
	WHERE t.people_id = pp.id AND pp.camp_id = '.$_SESSION['camp']['id'].' AND t.product_id > 0 AND t.transaction_date >= "'.$start.' 00:00" AND t.transaction_date <= "'.$end.' 23:59"
