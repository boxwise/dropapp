<?php
	...
	if(!$ajax) {
		...
		$data = getlistdata('SELECT cms_users.*, NOT is_admin AS visible, GROUP_CONCAT(c.name ORDER BY c.seq SEPARATOR ", ") AS camps FROM cms_users INNER JOIN cms_users_camps AS x ON x.cms_users_id = cms_users.id LEFT OUTER JOIN camps AS c ON c.id = x.camps_id '.($_SESSION['user']['is_admin']?'':' WHERE NOT is_admin').' GROUP BY cms_users.id');
