<?php
	...
	(SELECT IF(status="out",CONCAT((SELECT CONCAT(firstname," ",lastname," &nbsp;<a href=\"?action=people_edit&origin=borrow&id=",id,"\" class=\"fa fa-external-link\"></a>") FROM people WHERE id = people_id),IF(t.lights," ***","")),b.comment) FROM borrow_transactions AS t WHERE t.bicycle_id = b.id ORDER BY transaction_date DESC LIMIT 1) AS user, 
	(SELECT IF(status="out",
	
	IF(TIME_TO_SEC(TIMEDIFF(NOW(),transaction_date))>11000,CONCAT("<b class=\"warning\">",DATE_FORMAT(TIMEDIFF(NOW(),transaction_date),"%H:%i")," <i class=\"fa fa-warning\"></i></b>"),DATE_FORMAT(TIMEDIFF(NOW(),transaction_date),"%H:%i"))
