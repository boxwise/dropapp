<?php
	...
	$cmsmain->assign('title', strftime('%A %d %B %Y', strtotime('+'.$data['day'].' days', strtotime($cyclestart))).'<br />'.db_value('SELECT label FROM laundry_times WHERE id = :time',array('time'=>$data['time'])).' <span class="machine">'.db_value('SELECT label FROM laundry_machines WHERE id = :machine',array('machine'=>$data['machine'])).'</span>');
