...
(SELECT transaction_date FROM borrow_transactions AS t2 WHERE t.bicycle_id = t2.bicycle_id AND t2.transaction_date > t.transaction_date ORDER BY transaction_date LIMIT 1) AS datein, CONCAT(p.firstname," ",p.lastname) AS name FROM borrow_transactions AS t LEFT OUTER JOIN people AS p ON p.id = t.people_id WHERE bicycle_id = '.intval($_GET['id']).' AND status = "out" ORDER BY transaction_date DESC');
