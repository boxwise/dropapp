<?
	...
	if(!$row) $row = db_row('SELECT *, "family" AS usertype, CONCAT(firstname," ",lastname) AS naam FROM people WHERE email != "" AND email = :email AND NOT deleted',array('email'=>$_POST['email']));
	...
	if($row) { #e-mailaddress exists in database
		if($settings['local_adminonly'] && !$row['is_admin'] && $_SERVER['Local']) {
			...
			$redirect = false;
			logfile('Inlog op de local-website tegengehouden voor '.$_POST['email']);
		} elseif($row['pass']==$_POST['pass']) { # password is correct
			$success = true;
			$message = '';
			
			if($row['usertype']=='family') {
				$redirect = $flipdir.'/?action=status';	
			...
			logfile('Gebruiker ingelogd met '.$_SERVER['HTTP_USER_AGENT']);
			...
			$redirect = false;
			logfile('Poging tot inloggen met verkeerd wachtwoord door '.$_POST['email']);
		}
	} else { # user not found
		$success = false;
		$message = translate('cms_login_error_usernotfound');		
		$redirect = false;
		logfile('Poging tot inloggen door onbekende gebruiker '.$_POST['email']);
	}
