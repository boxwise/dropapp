...
*/
	...
	while($row1 = db_fetch($result1)) {
		...
		if($_SESSION['user']['is_admin']) {
			...
			$result2 = db_query('SELECT *, title_en AS title FROM (cms_functions AS f) LEFT OUTER JOIN cms_access AS x ON f.id = x.cms_functions_id LEFT OUTER JOIN cms_users AS u ON u.id = x.cms_users_id LEFT OUTER JOIN cms_functions_camps AS x2 ON x2.cms_functions_id = f.id WHERE f.visible AND (x2.camps_id = :camp OR f.allusers) AND (u.id = :user_id OR f.allusers) AND (f.parent_id = :parent_id) ORDER BY seq',array('camp'=>$_SESSION['camp']['id'],'parent_id'=>$row1['id'],'user_id'=>$_SESSION['user']['id']));
		}
