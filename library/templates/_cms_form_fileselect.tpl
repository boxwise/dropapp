	<div class="form-group" id="div_{$element['field']}">
		...
		<div class="col-sm-10">
			<a href="{$settings['rootdir']}/filemanager/dialog.php?type={if $element['filetype']=='image'}1{else}2{/if}{if $element['folder']}&fldr={$element['folder']}{/if}&field_id=field_{$element['field']}" class="btn btn-default popupfilemanager">{if $element['filetype']=='image'}{$translate['cms_form_chooseimage']}{else}{$translate['cms_form_choosefile']}{/if}</a>
