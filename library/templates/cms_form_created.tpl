...
{if $data['modified']}
	...
	{if $settings['showhistory']}<br /> <br /><i class="fa fa-history"></i> <a href="{$settings['rootdir']}/ajax.php?file=history&table={if $table}{$table}{else}{$smarty.get.action}{/if}&id={$data['id']}" class="fancybox" data-fancybox-type="ajax">{$translate['cms_form_view_modified']}</a>{/if}
{/if}
</div>